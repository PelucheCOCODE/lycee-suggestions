const HEAT_COLORS = {
    hot:  { pin: "#ef4444", bg: "rgba(239,68,68,0.10)", border: "rgba(239,68,68,0.3)", glow: "0 0 20px rgba(239,68,68,0.2)" },
    warm: { pin: "#f59e0b", bg: "rgba(245,158,11,0.08)", border: "rgba(245,158,11,0.25)", glow: "0 0 14px rgba(245,158,11,0.15)" },
    cool: { pin: "#6366f1", bg: "rgba(99,102,241,0.06)", border: "rgba(99,102,241,0.2)", glow: "none" },
};
...
async function checkDisplayMode() {
    try {
        ...
        busSettings = {
            ...
            bus_alternance_interval_sec: settings.bus_alternance_interval_sec || 60,
            feature_bus_enabled: settings.feature_bus_enabled !== false,
        };
        const newMode = settings.display_mode || "normal";

        if (newMode !== currentMode) {
            currentMode = newMode;
            applyDisplayMode(settings);
        } else if (newMode === "waiting") {
            waitingTitleEl.textContent = settings.display_waiting_title || "Boîte à Idées";
            waitingTextEl.textContent = settings.display_waiting_text || "";
        }
    } catch (e) { /* ignore */ }
}
...
async function fetchCompletedSuggestions() {
    ...
    try {
        ...
        const data = await res.json();
        updateCompletedBanner(data);
    } catch (e) { /* ignore */ }
}

function updateCompletedBanner(completed) {
    const banner = document.getElementById("display-completed-banner");
    const stack = document.querySelector(".display-banners-stack");
    if (!banner) return;
    if (!completed || !completed.length) {
        banner.classList.add("hidden");
        banner.innerHTML = "";
        if (stack) {
            const critical = document.getElementById("display-critical-banner");
            if (critical?.classList.contains("hidden")) stack.style.display = "none";
        }
        return;
    }
    banner.classList.remove("hidden");
    if (stack) stack.style.display = "";
    const html = completed.map((s) =>
        `La suggestion « ${escapeHtml(s.title)} » ne reçoit plus de vote, car : ${escapeHtml(s.reason)}`
    ).join(" · ");
...
async function fetchSuggestions() {
    ...
    try {
        const [suggestionsRes, proposalRes, criticalRes] = await Promise.all([
            fetch("/api/display/suggestions"),
            fetch("/api/official-proposal"),
            fetch("/api/display/critical-moments"),
        ]);
        const suggestions = await suggestionsRes.json();
        const proposal = await proposalRes.json();
        const critical = await criticalRes.json();
        updateDisplay(suggestions, proposal, critical);
    } catch (e) {
        console.error(e);
    }
}

function updateDisplay(suggestions, proposal, critical) {
    const total = suggestions.length + (proposal ? 1 : 0);
    countEl.textContent = `${total} element${total !== 1 ? "s" : ""}`;

    updateCriticalBanner(critical);
    updateProposalCard(proposal);

    const existingIds = new Set(currentData.map((s) => s.id));
    const newIds = new Set(suggestions.map((s) => s.id));

    suggestions.forEach((s, i) => {
        const card = document.getElementById(`display-card-${s.id}`);
        if (card) {
            updateCard(card, s);
            card.style.order = i;
        } else {
            const newCard = createCard(s, i);
    ...
    existingIds.forEach((id) => {
        if (!newIds.has(id)) {
            const card = document.getElementById(`display-card-${id}`);
            if (card) {
                card.classList.add("display-card-exit");
...
function updateCriticalBanner(critical) {
    const banner = document.getElementById("display-critical-banner");
    const stack = document.querySelector(".display-banners-stack");
    if (!banner) return;
    if (!critical || !critical.length) {
        ...
        banner.innerHTML = "";
        if (stack) {
            const completed = document.getElementById("display-completed-banner");
            if (completed?.classList.contains("hidden")) stack.style.display = "none";
        }
        return;
    }
    banner.classList.remove("hidden");
    if (stack) stack.style.display = "";
    const html = critical.map((c) => `
...
function updateProposalCard(proposal) {
    const grid = document.getElementById("display-grid");
    if (!grid) return;
    let card = document.getElementById("display-proposal-card");
    if (!proposal) {
        if (card) card.remove();
        return;
    }
    if (card) {
        card.querySelector(".cvl-content").innerHTML = proposal.content || "";
        const votesInline = card.querySelector(".cvl-votes-inline");
        if (votesInline) {
            votesInline.innerHTML = proposal.needs_debate
        ...
        const statusEl = card.querySelector(".cvl-status");
        if (statusEl) statusEl.textContent = proposal.status;
    } else {
        card = document.createElement("div");
        card.id = "display-proposal-card";
        card.className = "display-card display-card-cvl";
        card.style.order = "-1";
        card.innerHTML = `
...
function createCard(s, index) {
    const heat = HEAT_COLORS[s.heat] || HEAT_COLORS.cool;
    const icon = CATEGORY_ICONS[s.category] || "📌";

    const card = document.createElement("div");
    card.id = `display-card-${s.id}`;
    card.className = "display-card display-card-enter";
    card.style.order = index;
    card.style.background = heat.bg;
    card.style.borderColor = heat.border;
    card.style.boxShadow = heat.glow;
    ...
    requestAnimationFrame(() => card.classList.remove("display-card-enter"));
    return card;
}
...
function updateCard(card, s) {
    ...
    const pin = card.querySelector(".display-card-pin");
    if (pin) pin.style.background = heat.pin;

    const icon = CATEGORY_ICONS[s.category] || "📌";
    const titleEl = card.querySelector(".display-card-title");
    if (titleEl) titleEl.innerHTML = `${icon} ${escapeHtml(s.title)}`;

    let subEl = card.querySelector(".display-card-subtitle");
    if (s.subtitle) {
        if (!subEl) {
            subEl = document.createElement("div");
            subEl.className = "display-card-subtitle";
            titleEl?.parentNode?.insertBefore(subEl, titleEl.nextSibling);
        }
        subEl.textContent = s.subtitle;
    } else if (subEl) subEl.remove();

    const catEl = card.querySelector(".display-card-category");
    if (catEl) catEl.textContent = s.category;

    const statusEl = card.querySelector(".display-card-status");
    if (statusEl) { statusEl.textContent = s.status; statusEl.dataset.status = s.status; }

    const meta = card.querySelector(".display-card-meta");
    let locEl = card.querySelector(".display-card-location");
    if (s.location_name) {
        if (!locEl && meta) {
            locEl = document.createElement("span");
            locEl.className = "display-card-location";
            meta.appendChild(locEl);
        }
        if (locEl) locEl.textContent = `📍 ${s.location_name}`;
    } else if (locEl) locEl.remove();

    const voteCount = card.querySelector(".display-card-vote-count");
    if (voteCount && voteCount.textContent !== String(s.vote_count)) {
        voteCount.textContent = s.vote_count;
        voteCount.classList.add("vote-bump");
        setTimeout(() => voteCount.classList.remove("vote-bump"), 600);
    }

    const voteLabel = card.querySelector(".display-card-vote-label");
    if (voteLabel) voteLabel.textContent = `soutien${s.vote_count !== 1 ? "s" : ""}`;
}
...
const busPanel = document.getElementById("display-bus-panel");
const busStopsEl = document.getElementById("display-bus-stops");
let busSettings = null;
let showBusNow = false;
...
function checkAlternance() {
    ...
    if (now - lastAlternanceSwitch >= interval) {
        ...
        if (alternanceShowBus) updateBusDisplay();
        else switchToSuggestions();
    }
}
...
let suggestionsRefreshInterval = 6000;   // 6 sec for suggestions (actualisation en direct)
let settingsPollInterval = 3000;  // 3 sec for settings (toggles admin)
let lastBusRefresh = 0;
let lastSuggestionsRefresh = 0;
...
setInterval(async () => {
    ...
    } else if (showBusNow && busSettings?.bus_alternance_enabled) {
        if (now - lastBusRefresh >= busRefreshInterval) {
            lastBusRefresh = now;
            await updateBusDisplay();
        }
    } else {
        if (now - lastSuggestionsRefresh >= suggestionsRefreshInterval) {
            lastSuggestionsRefresh = now;
            fetchSuggestions();
            fetchCompletedSuggestions();
        }
    }
